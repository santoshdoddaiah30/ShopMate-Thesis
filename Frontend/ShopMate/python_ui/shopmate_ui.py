...
-----------------------------------------
Find the two functions marked `# TODO: PLUG YOUR BACKEND`
below and replace their bodies with calls to your own code:
    1) `answer_user_query(user_message, chat_history)`
       -> returns (assistant_text: str, products: list[dict])
    2) `get_trending_products()` and `get_categories()` (optional)
...
def answer_user_query(
    ...
    reply = (
        ...
        "I focused on top-rated options at different price points — let me know "
        "your budget or preferred brand and I'll narrow it down."
    )
...
def get_categories() -> List[str]:
    ...
    return ["Electronics", "Home & Kitchen", "Books", "Fashion", "Beauty", "Toys"]
...
def on_send(message: str, history: List[Dict[str, str]]):
    """Handle a user message. Returns updated (history, products_html, input_reset)."""
    ...
    with gr.Blocks(
        ...
        with gr.Row(equal_height=False):
            ...
            with gr.Column(scale=3, elem_id="sm-main"):
                ...
                # Suggestion chips (only visible-ish; always rendered)
