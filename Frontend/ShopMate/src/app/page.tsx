...
export default function HomePage() {
  ...
  const [authenticationGreeting, setAuthenticationGreeting] =
    useState<string | null>(null);
        ...
        : [
            authenticationGreeting
              ? {
                  ...
                  content: authenticationGreeting,
                }
              : welcomeMessage,
          ];
  ...
  async function handleLogin(
    ...
    try {
      ...
      setWorkspaceData(response.workspace);
      setAuthenticationGreeting(
        `Welcome back, ${normalizeShopMateWorkspace(response.workspace).displayName}!`,
      );
      setLoginPassword("");
    } catch (error) {
      setErrorMessage(
  ...
  async function handleRegistration(
    ...
    try {
      ...
      setWorkspaceData(response.workspace);
      setAuthenticationGreeting(
        `Hi, ${normalizeShopMateWorkspace(response.workspace).displayName}!`,
      );
      setRegisterPassword("");
      setRegisterConfirmPassword("");
    } catch (error) {
  ...
  async function handleLogout() {
    ...
    try {
      ...
      setToken(null);
      setWorkspaceData(null);
      setAuthenticationGreeting(null);
      setMessageInput("");
      ...
      <section className="shopmate-main">
        ...
        >
          <div className="conversation-inner">
            ...
            {workspace.generationSeconds !== null && (
              <div className="shopmate-generation-time">
                Generated in {workspace.generationSeconds.toFixed(1)} seconds
              </div>
            )}
            ...
            {/* =================================================
                N18Q — GROUPED OUTFIT REACT UI
                ================================================= */}

            {workspace.outfits.length > 0 && (
              <section className="outfit-recommendation-section">
                <div className="recommendation-heading">
                  <div>
                    <h2>Complete outfit recommendations</h2>

                    <p>
                      Coordinated looks using real products from
                      your processed Amazon catalogue
                    </p>
                  </div>

                  <span>
                    {workspace.outfits.length} complete looks
                  </span>
                </div>

                <div className="outfit-look-list">
                  {workspace.outfits.map((outfit) => (
                    <section
                      className="outfit-look-card"
                      key={`outfit-${outfit.rank}`}
                    >
                      <header className="outfit-look-header">
                        <div>
                          <div className="outfit-look-number">
                            LOOK {outfit.rank}
                          </div>

                          <h3>{outfit.title}</h3>

                          <div className="outfit-look-subtitle">
                            Five-piece complete outfit
                          </div>
                        </div>

                        <div className="outfit-budget-summary">
                          <strong>
                            {outfit.totalPrice !== null
                          ...
                          {outfit.remainingBudget !== null && (
                            <small>
                              ${outfit.remainingBudget.toFixed(2)}
                              {" "}remaining
                            </small>
                          )}
                        </div>
                      ...
                      <div className="outfit-product-grid">
                        {outfit.products.map((product) => (
                          ...
                          >
                            <div className="outfit-slot-label">
                              {product.slotLabel}
                            </div>

                            <div className="outfit-product-image-wrap">
                              {product.imageUrl ? (
                                <img
                                  className="outfit-product-image"
                            ...
                            <div className="outfit-product-information">
                              {product.brand && (
                              ...
                              <h4 className="outfit-product-title">
                                {product.title}
                              </h4>
                              ...
                              <div className="outfit-product-footer">
                                <div>
                                  <div className="outfit-product-price">
                                    {product.priceDisplay ??
                                      (product.price !== null
                                        ? `$${product.price.toFixed(2)} (historical)`
                                ...
                                {product.purchaseUrl ? (
                                  ...
                                  <span className="outfit-link-unavailable">
                                    Link unavailable
                                  </span>
                                )}
                              </div>
                ...
                <p className="historical-price-note">
                  Prices and catalogue information are historical
                  Amazon 2023 data and may not represent current
